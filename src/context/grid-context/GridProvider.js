...
const GridProvider = (props) => {
   ...
   const clearNodes = (type, actionedByUser) => {
      setGrid((grid) => {
         ...
         if(actionedByUser) {
            messageContext.createMessage("Clearing " + pointTypeText, MessageType.PURPLE, 3000);
         }
         return updatedGrid;
      });
   };
   ...
   const visualizeMaze = async (maze) => {
      ...
      setVisualizing(true);
      messageContext.createMessage("Visualizing " + mazeText, MessageType.TURQUOISE, 3000);
      await algorithm(dimensionsContext, startCoord, targetCoord, controlContext, setGrid, speed);
      messageContext.createMessage("Finished Visualizing " + mazeText, MessageType.GREEN, 3000);
   ...
   const visualizeAlgorithm = async () => {
      ...
      setVisualizing(true);
      messageContext.createMessage("Visualizing " + algorithmText, MessageType.TURQUOISE, 3000);
      await algorithm(grid, dimensionsContext, startCoord, targetCoord, controlContext, setGrid, speed);
      messageContext.createMessage("Finished Visualizing " + algorithmText, MessageType.GREEN, 3000);
      setVisualizing(false);
   };
